$(document).ready(function () {
    ...
    */
    ...
    function generateSnippit() {
        ...
        if (!error) {
            if ($username.val() != '') {
                snippit = '&lt;iframe src="'+ siteUrl + ($slideShow.val().toLowerCase() == 'yes' ? 'sl' : 'in') + '/?u=';
                id = $username.val().toLowerCase();
            } else {
                snippit = '&lt;iframe src="'+ siteUrl + ($slideShow.val().toLowerCase() == 'yes' ? 'sl' : 'in') + '/?h=';
                id = $hashtag.val().toLowerCase();
            }
              
            width = parseInt($layoutX.val()) * (parseInt($thumbnailSize.val()) + parseInt($photoPadding.val()) + ($photoBorder.val() == 'yes' ? 10 : 5));
